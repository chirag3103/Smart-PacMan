...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        listFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #parsing through the elimination order
        for var in eliminationOrder:
            #initializing the factor variables
            listFactors, newFactor = joinFactorsByVariable(listFactors, var)
            #elimating the factors with len > 1
            if len(newFactor.unconditionedVariables()) > 1:
                tempFactor = eliminate(newFactor, var)
                #creating a new list of factors with eliminated vars
                listFactors.append(tempFactor)
        #appending the factors
        joinedFactor = joinFactors(listFactors)
        #returning the normalized factors
