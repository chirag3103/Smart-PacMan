...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #for both top probabilities
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)
    #for both bottom probabilities
    yFactor.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL}, PROB_BOTH_BOTTOM)
    #for top left probabilities
    yFactor.setProbability({Y_POS_VAR: LEFT_TOP_VAL}, PROB_ONLY_LEFT_TOP)
    #for  bottom left probabilities
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #getting all the possible houses
    legalhouseLoc = gameState.getPossibleHouses()
    #obtaining full set of observations
    #parsing through every house location
    for currenthousePos in legalhouseLoc:
        for currentobsPos in gameState.getHouseWalls(currenthousePos):
            currentobsVar = OBS_VAR_TEMPLATE % currentobsPos
            
            obsVarFactor = bn.Factor([currentobsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            obsCPT = obsVarFactor.getAllPossibleAssignmentDicts()
            #parsing through each CPT in the obsCPT
            for eachCPT in obsCPT:
                #assigning the ghost and food house values
                ghostHouseVal = eachCPT[GHOST_HOUSE_VAR]
                foodHouseVal = eachCPT[FOOD_HOUSE_VAR]
                #assigning the color with the current observed value color
                currColor = eachCPT[currentobsVar]
                #initializing a quadrant as a block variable
                block = getAdjQuadrant(gameState, currentobsPos)
                prob = 0
                #checking if the block is not a ghost not a food house
                if block != ghostHouseVal and block != foodHouseVal:
                    if currColor == RED_OBS_VAL:
                        prob = 0
                    elif currColor == BLUE_OBS_VAL:
                        prob = 0
                    elif currColor == NO_OBS_VAL:
                        prob = 1
                #checking if the block is a food house or a ghost house
                elif ghostHouseVal == foodHouseVal or foodHouseVal == block:
                    if currColor == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif currColor == BLUE_OBS_VAL:
                        ...
                        prob = 0
                #checking if the block is a ghost house
                elif block == ghostHouseVal:
                    if currColor == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif currColor == BLUE_OBS_VAL:
                        ...
                        prob = 0
                #setting the probability of the CPT
                obsVarFactor.setProbability(eachCPT, prob)
            #setting the CPT in the bayes network
            bayesNet.setCPT(currentobsVar, obsVarFactor)    
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    topfoodGrid = None
    prob = 0
    #parsing through the dictionary of assignments
    for legalAssignment in query_factor.getAllPossibleAssignmentDicts():
        #calculating the probability value for each assignment
        probVal = query_factor.getProbability(legalAssignment)
        #if the calculated probability is higher than the set probability
        if probVal > prob:
            #reset the set probability
            prob = probVal
            #set the food house with thr current assignment
            topfoodGrid = legalAssignment
    return topfoodGrid
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        #util.raiseNotDefined()
        from inference import inferenceByVariableElimination
        queryFactor = inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        ...
        leftExpectedValue = leftPFood * WON_GAME_REWARD + (1  - leftPFood) * GHOST_COLLISION_REWARD
        rightExpectedValue = rightPFood * WON_GAME_REWARD + (1 - rightPFood) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            evidenceNew = evidence.copy()
            evidenceNew.update(explorationEvidence)
            left, right = self.computeEnterValues(evidenceNew, enterEliminationOrder)
            chance_enter_val = max(left, right)
            expectedValue += chance_enter_val * prob
